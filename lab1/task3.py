...
def generate_sensors(radius: float = 2.0) -> List[np.ndarray]:
    ...
    for i in range(L):
        ...
        sensors.append(np.array([radius * np.cos(angle), radius * np.sin(angle), h]))
    return sensors
...
def generate_data(
    T: int, sensors: List[np.ndarray], lambda_p: float, delta_q: float, R: np.ndarray
...
class Particle:
    ...
    def __init__(
        self, p: np.ndarray, q_mean: np.ndarray, q_cov: np.ndarray, weight: float
    ) -> None:
        """
